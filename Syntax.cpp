...
void SyntaxAnalyser::construct() {
    ...
    if (_lex[_ind].string != "{") throw std::logic_error("\"{\" expected");
    gc();
    _list->AddMethod(object_name,"CONSTRUCTOR", _gen->GetCurSize());
    //_tid->OpenScope();
    namepace();
    _tid->CloseScope();
    if (_lex[_ind].string != "}") throw std::logic_error("\"}\" expected");
...
void SyntaxAnalyser::expression() {
    if (_lex[_ind].type == "number") {
        ...
        _gen->Push(new PolizSymbol(_lex[_ind].string));
    ...
    if (_lex[_ind].type == "variable" || _lex[_ind].type == "number" || _lex[_ind].type == "char") {
        //_gen->Push(new PolizGet(_lex[_ind].string, _tid->GetType(_lex[_ind].string)));
        gc();
        ...
        if (_lex[_ind].string == "(") {
            //_gen->Erase();
            std::string func_name = _lex[_ind - 1].string;
        ...
        while (_lex[_ind].string == ".") {
            ...
            _gen->Push(new PolizOperator(_point, -1));
    ...
    if (_lex[_ind].type == "unary") { // rewritten
        ...
        gc();
        if (_lex[_ind - 1].type == "-") {
            _gen->Push(new PolizOperator((std::string&)"~", _lex[_ind - 1].priority));
        } else {
            _gen->Push(new PolizOperator(_lex[_ind - 1].string, _lex[_ind - 1].priority));
        }
        if (_lex[_ind].type != "variable") throw std::logic_error("variable expected");
        auto type1 = _tid->GetType(_lex[_ind].string);
        expCheck.Process(Lexeme("variable", type1, 0));
        Lexeme exp_last = expCheck.GetLast();
        _gen->Push(new PolizGet(_lex[_ind].string, type1));
        gc();
        //array check
        if (_lex[_ind].string == "[") {
            _gen->Push(new PolizBracket('['));
        ...
        if (_lex[_ind].string != ";") {
           ...
           " in " + return_type + " function");
            ...
            if (_tid->GetCurrentReturnType() != "void") {
                throw std::logic_error("return expression expected in non-void type function");
            }
        }
        if (_lex[_ind].string != ";") {
            throw std::logic_error("\";\" expected");
...
void SyntaxAnalyser::lexpression() {
    ...
    if (type()) {
        ...
        if (_tid->IsFunctionExist(_lex[_ind].string)) {
            ...
            if (_lex[_ind].type == "variable") {
                ...
                var_type = type1 = _lex[_ind - 1].string;
                def = true;
                gc();
                ...
                _gen->Push(new PolizGet(name, type1));
            }
        }
    ...
    while (_lex[_ind].string == ".") {
        ...
        if (_lex[_ind].string == "(") {
            _parameter_arr = _tid->GetMethodParameters(type1, _lex[_ind - 1].string);
            parameters();
            _gen->AddMethod(_parameter_arr.size(), method, type1);
            _parameter_arr.clear();
            if (_lex[_ind].string != ")") {
                throw std::logic_error("\")\" expected");
            }
            ...
            _gen->Push(new PolizOperator(_point, -1));
            _gen->Push(new PolizGet(method, _tid->GetMember(type1, method)));
...
void SyntaxAnalyser::type_cast() {
    ...
    _gen->Push(new PolizGet(_lex[_ind].string, type2));
    _gen->AddCast(type1, type2);
}
...
void SyntaxAnalyser::print() {
    ...
    if (_lex[_ind].string == ")") {
        gc();
        _gen->AddToRes(new PolizOutput(count));
        return;
...
void SyntaxAnalyser::input() {
    ...
    _gen->AddToRes(new PolizInput(count));
}
...
void SyntaxAnalyser::array_def() {
    ...
    _gen->AddToRes(new PolizAdd(arr_name, add_name));
    variable();
    if (_lex[_ind].string == "(") {
        gc();
        expression();
